...
请自行抓包，阅读文章和看视频，倒计时转一圈显示青豆到账即可，多看几篇文章和视频，获得更多包数据，抓包地址为"https://ios.baertt.com/v5/article/complete.json"，在Github Actions中的Secrets新建name为'YOUTH_READ_T'的一个值，拷贝抓包的请求体到下面Value的文本框中，添加的请求体越多，获得青豆次数越多，本脚本不包含任何推送通知
多个请求体时用'&'号或者换行隔开" ‼️
*/
...
if (!$.isNode() && !YouthBody == true) {
    ...
    if ($.isNode()) {
        if (process.env.YOUTH_READ_T && process.env.YOUTH_READ_T.indexOf('&') > -1) {
            YouthBodys = process.env.YOUTH_READ_T.split('&');
            console.log(`您选择的是用"&"隔开\n`)
        } else if (process.env.YOUTH_READ_T && process.env.YOUTH_READ_T.indexOf('\n') > -1) {
            YouthBodys = process.env.YOUTH_READ_T.split('\n');
            console.log(`您选择的是用换行隔开\n`)
        } else {
            YouthBodys = [process.env.YOUTH_READ_T]
        }
    } else if (!$.isNode() && YouthBody.indexOf("&") > -1) {
        ...
        $.post(batHost('article/complete.json', articlebody), async(error, response, data) => {
            ...
            if (readres.items.complete == 1) {
                ...
                if (readres.error_code == '0' && data.indexOf("read_score") > -1 && readres.items.read_score > 0) {
                    ...
                    if ($.index % 2 == 0) {
                        if ($.isNode() && process.env.YOUTH_ATIME_T) {
                            timebodyVal = process.env.YOUTH_ATIME_T;
                        } else {
